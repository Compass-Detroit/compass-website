...
const KNOWN_ALIASES = {
  'madona-s-wambua': 'madona-wambua',
  'dave-koziel': 'dave-koziol',
...
export function generateSlug(name) {
  ...
  return KNOWN_ALIASES[baseSlug] || baseSlug
}
  ...
  rawData.forEach(({ year, data }) => {
    ...
    data.forEach((speaker) => {
      ...
      if (speaker.avatar) {
        ...
        if (!exists) {
          profile.avatarHistory.push({ year, avatar: speaker.avatar })
        }
      }

      if (speaker.bio && speaker.bio.length > (profile.bio || '').length)
        profile.bio = speaker.bio
      ...
      const sessions = Array.isArray(speaker.session)
        ? speaker.session
        : speaker.session
          ? [speaker.session]
          : []
