...
import { FaDownload, FaSearch, FaFilter, FaUsers, FaChartLine, FaEnvelopeOpenText, FaCalendarAlt } from 'react-icons/fa'

const generateSampleData = () => {
  const interestsList = ['React', 'AI', 'Design', 'Startups', 'Career', 'Backend', 'Events']
  const names = ['John Doe', 'Jane Smith', 'Alice Johnson', 'Bob Williams', 'Charlie Brown', 'David Davis', 'Eva Garcia', 'Frank Miller', 'Grace Lee', 'Hannah Wilson', 'Ian Taylor', 'Julia White', 'Kevin Harris', 'Laura Martin', 'Michael Thompson', 'Nina Moore', 'Oscar Clark', 'Paul Lewis', 'Quinn Walker', 'Rachel Hall']
  const statuses = ['active', 'active', 'active', 'active', 'unsubscribed']
  
  return Array.from({ length: 20 }).map((_, i) => {
    const numInterests = Math.floor(Math.random() * 3) + 1
    const userInterests = [...interestsList].sort(() => 0.5 - Math.random()).slice(0, numInterests)
    const date = new Date()
...
const SubscriberDashboardPage = () => {
  ...
  useEffect(() => {
    ...
    if (stored) {
      try {
        ...
        localStorage.setItem('compass_newsletter_subscribers', JSON.stringify(sample))
      }
    } else {
      const sample = generateSampleData()
      setSubscribers(sample)
  ...
  const filteredSubscribers = useMemo(() => {
    return subscribers.filter(sub => {
      ...
      const matchesInterest = filterInterest ? sub.interests.includes(filterInterest) : true
  ...
  const stats = useMemo(() => {
    ...
    const topInterest = Object.entries(interestsCount).sort((a, b) => b[1] - a[1])[0]?.[0] || 'N/A'

    return { total: subscribers.length, active, thisMonth, topInterest, interestsCount }
  }, [subscribers])
  ...
  const allInterests = Array.from(new Set(subscribers.flatMap(s => s.interests)))
        ...
        <div className="relative mb-8 bg-white py-12 shadow-sm dark:bg-gray-800">
          ...
          <div className="relative z-10 mx-auto max-w-7xl px-4 sm:px-6 lg:px-8">
            <div className="flex flex-col items-start justify-between gap-4 md:flex-row md:items-center">
              <div>
                <h1 className="text-3xl font-bold text-gray-900 dark:text-white">Newsletter Analytics</h1>
                <p className="mt-2 text-gray-600 dark:text-gray-400">Manage and analyze your subscriber base.</p>
              </div>
              <button 
        ...
        <div className="mx-auto max-w-7xl px-4 sm:px-6 lg:px-8">
          ...
          <div className="mb-8 grid grid-cols-1 gap-4 sm:grid-cols-2 lg:grid-cols-4">
            ...
            <div className="rounded-xl bg-white p-6 shadow-sm ring-1 ring-gray-200 dark:bg-gray-800 dark:ring-gray-700">
              <div className="flex items-center gap-4">
                ...
                <div>
                  <p className="text-sm font-medium text-gray-500 dark:text-gray-400">Top Interest</p>
                  <p className="text-2xl font-bold text-gray-900 dark:text-white">{stats.topInterest}</p>
                </div>
              </div>
            </div>
          ...
          <div className="grid grid-cols-1 gap-8 lg:grid-cols-3">
            ...
            <div className="lg:col-span-2">
              <div className="rounded-xl bg-white p-6 shadow-sm ring-1 ring-gray-200 dark:bg-gray-800 dark:ring-gray-700">
                <div className="mb-6 flex flex-col gap-4 sm:flex-row sm:items-center sm:justify-between">
                  <h2 className="text-lg font-bold text-gray-900 dark:text-white">Subscriber List</h2>
                        ...
                        filteredSubscribers.map((sub, idx) => (
                          <tr key={idx} className="hover:bg-gray-50 dark:hover:bg-gray-750/50">
                            <td className="px-4 py-3">
                              ...
                              <div className="text-gray-500 dark:text-gray-400">{sub.email}</div>
                            </td>
                            <td className="px-4 py-3">
                              <div className="flex flex-wrap gap-1">
                                {sub.interests.map(interest => (
                                  <span key={interest} className="inline-flex rounded-full bg-indigo-100 px-2 py-0.5 text-xs font-medium text-indigo-800 dark:bg-indigo-900/30 dark:text-indigo-300">
                                    {interest}
                                  </span>
                                ))}
                              </div>
            ...
            <div className="flex flex-col gap-8">
              ...
              {/* Interest Breakdown */}
              <div className="rounded-xl bg-white p-6 shadow-sm ring-1 ring-gray-200 dark:bg-gray-800 dark:ring-gray-700">
                <h2 className="mb-6 text-lg font-bold text-gray-900 dark:text-white">Interest Breakdown</h2>
              ...
              <div className="rounded-xl bg-white p-6 shadow-sm ring-1 ring-gray-200 dark:bg-gray-800 dark:ring-gray-700">
                <h2 className="mb-6 text-lg font-bold text-gray-900 dark:text-white">Growth Overview</h2>
