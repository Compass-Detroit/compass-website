...
const backgrounds = [
  ...
  { id: 'bg-gradient-1', name: 'Gradient 1', class: 'bg-gradient-to-br from-primary to-pumpkin' },
  { id: 'bg-gradient-2', name: 'Gradient 2', class: 'bg-gradient-to-br from-gray-900 to-indigo-900' },
];
  ...
  return (
    <PageLayout>
      <div className="mx-auto max-w-7xl px-4 py-8 sm:px-6 lg:px-8 pt-24">
        <div className="mb-8">
          <h1 className="text-3xl font-bold text-gray-900 dark:text-white">Social Card Generator</h1>
          <p className="text-gray-600 dark:text-gray-400">Generate branded social media assets for speakers and events.</p>
        </div>
        ...
        <div className="flex flex-col gap-8 lg:flex-row">
          ...
          <div className="w-full lg:w-1/3 space-y-6 rounded-2xl bg-white p-6 shadow-md dark:bg-gray-800">
            ...
            <div>
              <div className="mb-4 flex items-center justify-between">
                <label className="block text-sm font-medium text-gray-700 dark:text-gray-300">Content Source</label>
                <button 
              ...
              {!manualMode ? (
                ...
                <div className="space-y-3">
                  <input type="text" placeholder="Speaker Name" className="w-full rounded-md border-gray-300 dark:border-gray-600 dark:bg-gray-700 dark:text-white text-sm" value={customData.name} onChange={e => setCustomData({...customData, name: e.target.value})} />
                  <input type="text" placeholder="Talk Title" className="w-full rounded-md border-gray-300 dark:border-gray-600 dark:bg-gray-700 dark:text-white text-sm" value={customData.talkTitle} onChange={e => setCustomData({...customData, talkTitle: e.target.value})} />
                  <input type="text" placeholder="Organization" className="w-full rounded-md border-gray-300 dark:border-gray-600 dark:bg-gray-700 dark:text-white text-sm" value={customData.organization} onChange={e => setCustomData({...customData, organization: e.target.value})} />
                  <input type="text" placeholder="Avatar URL" className="w-full rounded-md border-gray-300 dark:border-gray-600 dark:bg-gray-700 dark:text-white text-sm" value={customData.avatarUrl} onChange={e => setCustomData({...customData, avatarUrl: e.target.value})} />
                </div>
              )}
            </div>
            ...
            <div className="space-y-4">
              ...
              <div className="grid grid-cols-2 gap-4">
                <div>
                  <label className="mb-1 block text-sm text-gray-600 dark:text-gray-400">Font Size</label>
                  <select 
                    ...
                    onChange={(e) => setDesignOptions({...designOptions, fontSize: e.target.value})}
                  >
                ...
                <div>
                  <label className="mb-1 block text-sm text-gray-600 dark:text-gray-400">Logo Position</label>
                  <select 
                    className="w-full rounded-md border-gray-300 text-sm dark:border-gray-600 dark:bg-gray-700 dark:text-white"
                    value={designOptions.logoPosition}
                    onChange={(e) => setDesignOptions({...designOptions, logoPosition: e.target.value})}
                  >
              ...
              <div className="flex items-center gap-4 text-sm text-gray-700 dark:text-gray-300">
                <label className="flex items-center gap-2">
                  <input type="checkbox" checked={designOptions.showTrack} onChange={(e) => setDesignOptions({...designOptions, showTrack: e.target.checked})} className="rounded text-primary focus:ring-primary" />
                  Show Track
                </label>
                <label className="flex items-center gap-2">
                  <input type="checkbox" checked={designOptions.showYear} onChange={(e) => setDesignOptions({...designOptions, showYear: e.target.checked})} className="rounded text-primary focus:ring-primary" />
                  Show Year
                </label>
              </div>
            </div>
            ...
            <div className="pt-4">
              <p className="text-xs text-gray-500 italic mb-2">Note: Export functionality is coming soon. For now, please screenshot the preview.</p>
              <button disabled className="w-full rounded-lg bg-primary px-4 py-2 font-bold text-gray-900 opacity-50 cursor-not-allowed">
                Copy as Image
              </button>
            </div>
          </div>
          ...
          <div className="w-full lg:w-2/3 flex items-center justify-center overflow-auto rounded-2xl bg-gray-100 p-8 dark:bg-gray-900 min-h-[600px]">
            ...
            >
              ...
              {/* Logo */}
              <div className={`absolute ${getLogoPositionClass()} flex items-center gap-2 font-bold text-2xl tracking-tight`}>
              ...
              <div className="mt-16 flex flex-col items-center text-center">
                <img 
                  ...
                  alt={customData.name} 
                  className="mb-6 h-48 w-48 rounded-full border-4 border-white/20 object-cover shadow-xl bg-gray-300"
                />
                <h2 className={`${getFontSizeClass('name')} font-bold leading-tight`}>{customData.name}</h2>
                <p className={`${getFontSizeClass('subtitle')} font-medium opacity-80 mt-2`}>
                  {customData.position && <span>{customData.position}</span>}
                  {customData.position && customData.organization && <span> @ </span>}
                  {customData.organization && <span>{customData.organization}</span>}
                </p>
              </div>
              ...
              <div className="mb-10 w-full rounded-2xl bg-black/20 p-8 backdrop-blur-sm border border-white/10 text-center">
                <h1 className={`${getFontSizeClass('title')} font-extrabold leading-tight shadow-sm`}>
